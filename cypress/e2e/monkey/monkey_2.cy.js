...
const url = Cypress.config('baseUrl') || "http://localhost:3002/ghost"
const appName = Cypress.env('appName') || "Ghost 5.96"
const events = Cypress.env('events') || 100
const delay = Cypress.env('delay') || 100
var seed = Cypress.env('seed')
var events_count = Cypress.env('events')
...
function fullPath(el) {
    ...
    cy.task('genericLog', { "message": "Path: " + names.join(" > ") })
    return names.join(" > ");
}

function validarCadena(cadena) {
    // Encontrar la posición del primer '>'    
    const primerMayorQueIndex = String(cadena).indexOf('>');

    // Asegurarse de que existe '>'
    if (primerMayorQueIndex === -1) {
        return false; // No hay '>', por lo tanto no se cumple la condición
    }

    // Extraer la parte antes del primer '>'
    const antesDelPrimerMayorQue = cadena.slice(0, primerMayorQueIndex);

    // Contar cuántas veces aparece ':' en esa parte
    const countColons = (antesDelPrimerMayorQue.match(/:/g) || []).length;
    // Devolver true si hay más de un ':' y false en caso contrario
    return countColons > 1;
}
    ...
    cy.window().then((win) => {
        ...
        if (!!element) {
            ...
            if (!!element.id) { //boolean that indicates if the element has a non-empty id
                cy.task('genericLog', { "message": ">> Val element.id: " + element.id })
                cy.task('genericLog', { "message": ">> Val Pseudo element.id: " + validarCadena(element.id) })
            ...
            /*
                ...
                cy.task('genericLog', { "message": ">> Val jsPath: " + jsPath })
                cy.task('genericLog', { "message": ">> Val Pseudo jsPath:" + validarCadena(jsPath) })
                if (validarCadena(jsPath)) {
                    cy.task('genericLog', { "message": "Id pseudo inválido [:]" })
                } else {
...
function randomEvent() {
    ...
    if (pending_events[typeIndex] > 0) {
        ...
        cy.task('genericLog', { "message": "Event ini: " + String(functions[typeIndex][fIndex]).substring(0, 20) })
...
describe(`${appName} under monkeys`, function () {
    ...
    it(`visits ${appName} and survives monkeys`, function () {
        ...
        if (pcg === 100) {
            cy.task('genericLog', { "message": `Pcg: ${pcg}` })
            ...
            cy.visit(url).then((win) => {
                ...
                curPageMaxY = Math.max(d.body.scrollHeight, d.body.offsetHeight, d.documentElement.clientHeight, d.documentElement.scrollHeight, d.documentElement.offsetHeight) - win.innerHeight
                curPageMaxX = Math.max(d.body.scrollWidth, d.body.offsetWidth, d.documentElement.clientWidth, d.documentElement.scrollWidth, d.documentElement.offsetWidth) - win.innerWidth
                cy.task('genericLog', { "message": `Wait 2 sec..` })
                cy.wait(5000)
                cy.url().then((currentUrl) => {
                    // In this part we make sure that monkey log in on Ghost 5.96
                    cy.task('genericLog', { "message": `currentUrl: ${currentUrl}` })
                    if (currentUrl == url + '/#/signin') {
                        cy.task('genericLog', { "message": `Inicio: SignIn` })
                        cy.get('input[id="identification"]').type('prueba@prueba.com');
                        cy.get('input[id="password"]').type('prueba123456');
                        cy.get('button[data-test-button="sign-in"]').click();
                        cy.screenshot('screenshot_inicial');
                        //Add an event for each type of event in order to enter the else statement of randomEvent method
                    } else if (currentUrl == url + '/#/setup') {
                        cy.task('genericLog', { "message": `Inicio: Setup` })
                        cy.get('input[id="blog-title"]').type('MISO');
                        cy.get('input[id="name"]').type('Prueba Monkey');
                        cy.get('input[id="email"]').type('prueba@prueba.com');
                        cy.get('input[id="password"]').type('prueba123456');
                        cy.get('button[data-test-button="setup"]').click();
                        cy.screenshot('screenshot_inicial');
                    } else {
                        cy.task('genericLog', { "message": `Inicio: Sin Validacion` })
                        cy.task('stopTests');
                    }
                    cy.task('genericLog', { "message": `Wait 2 sec..` })
                    cy.wait(2000)
            ...
            for (let i = 0; i < events + 5; i++) {
                evtIndex++
                cy.task('genericLog', { "message": `Event: ${events_count} - %${(((events_count * 100 / events) - 100) * -1).toFixed(2)}` })
                events_count--
                randomEvent()
            }
        }
