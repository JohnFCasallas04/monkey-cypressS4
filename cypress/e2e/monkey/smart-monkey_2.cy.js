...
const LOG_FILENAME = Cypress.env('rutaLOG')
        ...
        if ($inputs.length > 0) {
            ...
            if (!Cypress.dom.isHidden(inp)) {
                ...
                if (inp.getAttribute("type") == "email") {
                    ...
                    let type = faker.string.alphanumeric();
                    cy.wrap(inp).type(type);
                    info = `Input ${inp.id} was filled with ${type}`
                }
                else {
...
describe(`${appName} under smarter monkeys`, function () {
    ...
    it(`visits ${appName} and survives smarter monkeys`, function () {
        ...
        if (pcg === 100) {
            ...
            cy.task('genericLog', { "message": `Wait 3 sec..` })
            cy.wait(3000)
            ...
            for (let i = 0; i < events + 7; i++) {
                ...
                evtIndex++
                cy.task('genericLog', { "message": `Event: ${events_count} - %${(((events_count * 100 / (events + 7)) - 100) * -1).toFixed(2)}` })
                events_count--
                randomEvent()
            }
        }
